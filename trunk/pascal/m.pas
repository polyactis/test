...
const     npopm=5; nindivm= 100; nlocm=150; alm=4;  {wich may be modified}
...
var       base                                   : giga;
         ...
         apop                                   : array [1..npopm] of string[12];
         aloc                                   : array [1..nlocm] of string[5];
         alec                                   : array [1..nlocm,1..alm] of integer;
         input,nomfreq,nomprob,a,b              : string[15];
...
begin
    assign (file1,input);
     reset  (file1);
    readln (file1,tzini,teqini);
    {lecture du nombre de pop et du nombre de locus}
    readln (file1,npop,nloc);writeln(npop,'  ',nloc);
...
begin
     ...
     {lecture des g‚notypes par pop,locus et couple d'allŠles}
...
begin
 assign (file4,outfreq);
 append (file4);
 Writeln(file4,'population nø ', cpop,' :  ',apop[cpop]);
 for loc:=1 to nloc do begin
...
BEGIN
     ...
     fin:=10; maille:=7; seuildeltavrais:=1.000001;
             ...
             nomfreq := input + '.fr';
             nomprob := input + '.ph';
             writeln('in: ',input,'- freqout: ', nomfreq, '-proba: ', nomprob);
             LECTUREDEB;
             OUVRPROB(nomprob);
             OUVRFREQ(nomfreq);
